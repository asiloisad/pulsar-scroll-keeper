...
module.exports = {
  activate() {
    ...
    this.editorPositions = new WeakMap();
    this.editorPreservers = new WeakMap();
    ...
    this.disposables.add(
      atom.workspace.observeTextEditors((editor) => {
              ...
              if (newScreenPosition !== null) {
                ...
                if (newPixelPos !== null) {
                  ...
                  if (this.editorAnchors) {
                    this.editorAnchors.set(editor, anchor);
                  }
                }
              }
            }
        ...
        const restoreSavedPositionSync = () => {
          const anchor = this.editorAnchors && this.editorAnchors.get(editor);
          restoreAnchor(anchor);
        };

        const capturePositionSync = () => {
          const anchor =
            captureViewportAnchor() || (this.editorAnchors && this.editorAnchors.get(editor));
          if (!anchor || anchor.bufferPosition === null) {
            return null;
          }

          const marker = editor.markBufferPosition(anchor.bufferPosition, {
            invalidate: "never",
          });
        ...
        const restorePositionSync = (position) => {
          restoreAnchor(position ? position.getAnchor() : null);
        };

        const preservePositionSync = (callback) => {
          const position = capturePositionSync();
        ...
        this.editorPositions.set(editor, {
          capturePosition: capturePositionSync,
          restorePosition: restorePositionSync,
        });
        this.editorPreservers.set(editor, preservePositionSync);

        // Restore the saved visual anchor after asynchronous layout changes.
        const restorePosition = () => {
          requestAnimationFrame(restoreSavedPositionSync);
        };
        ...
        // external integration: either wrap a synchronous change, or recover
        // position immediately after older integrations request it.
        const requestHandler = (request) => {
          if (request && typeof request.perform === "function") {
            request.handled = true;
            return preservePositionSync(request.perform);
          }
          restoreSavedPositionSync();
        };
        ...
        disposables.add(
          editor.onDidDestroy(() => {
            ...
            if (this.editorPositions) {
              this.editorPositions.delete(editor);
            }
            if (this.editorPreservers) {
              this.editorPreservers.delete(editor);
            }
            disposables.dispose();
  ...
  deactivate() {
    ...
    this.editorPositions = null;
    this.editorPreservers = null;
  },

  provideScrollKeeper() {
    return {
      capturePosition: (editor) => {
        const positions = this.editorPositions && this.editorPositions.get(editor);
        if (positions) {
          return positions.capturePosition();
        }
        return null;
      },
      restorePosition: (editor, position) => {
        const positions = this.editorPositions && this.editorPositions.get(editor);
        if (positions) {
          positions.restorePosition(position);
        }
      },
      preserveScroll: (editor, callback) => {
        const preserve = this.editorPreservers && this.editorPreservers.get(editor);
        if (preserve) {
