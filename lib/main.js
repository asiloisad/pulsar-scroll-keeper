...
module.exports = {
  activate() {
    ...
    this.sourceEditorByBuffer = new WeakMap();
      ...
      atom.workspace.observeTextEditors((editor) => {
        ...
        if (copiedEditorAnchor) {
          requestAnimationFrame(() => {
            ...
            if (copiedEditorAnchor.bufferRow !== null) {
              let newScreenRow = editor.screenRowForBufferRow(copiedEditorAnchor.bufferRow);
        ...
        const restorePosition = () => {
          requestAnimationFrame(() => {
            ...
            if (anchor && anchor.bufferRow !== null) {
              let newScreenRow = editor.screenRowForBufferRow(anchor.bufferRow);
              if (newScreenRow !== null) {
                let newPixelPos = component.pixelPositionAfterBlocksForRow(newScreenRow);
                if (newPixelPos !== null) {
        ...
        // Mark which editor initiated the current text insertion so sibling
        // editors can be adjusted without moving the editor being typed in.
        disposables.add(
          editor.onWillInsertText(() => {
            this.sourceEditorByBuffer.set(buffer, editor);
          }),
        );
        ...
        component.didChangeDisplayLayer = (changes) => {
          const sourceEditor = this.sourceEditorByBuffer.get(buffer);
          const isSourceEditor = sourceEditor === editor;
          let scrollDelta = 0;

          // Only sibling editors need compensation. The source editor should
          // keep its native typing/scroll behavior.
          if (!isSourceEditor && component.measurements) {
          ...
          // Let every editor observing this buffer see the same source marker
          // before clearing it at the end of the current synchronous change.
          if (isSourceEditor) {
            queueMicrotask(() => {
              if (
                this.sourceEditorByBuffer &&
                this.sourceEditorByBuffer.get(buffer) === sourceEditor
              ) {
                this.sourceEditorByBuffer.delete(buffer);
              }
            });
          }
        };
        ...
        const scrollPatch = debounce(() => {
          ...
          const pixelPos = component.pixelPositionAfterBlocksForRow(screenPosition);
          if (pixelPos === null) {
            return true;
          }
  ...
  deactivate() {
    ...
    this.sourceEditorByBuffer = null;
