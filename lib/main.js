...
module.exports = {
  ...
  activate() {
    ...
    this.disposables.add(atom.workspace.observeTextEditors((editor) => {
      ...
      disposables.add(editor.observeCursors(debounce((cursor) => {
        if (!cursor) { return }
      ...
      disposables.add(editor.onDidChangeCursorPosition(debounce((event) => {
        if (event.oldBufferPosition.compare(event.newBufferPosition) === 0) {
          return
        }
        const pixelPos = component.pixelPositionAfterBlocksForRow(event.newScreenPosition.row)
        if (pixelPos == null) { return }
      ...
      const scrollPatch = debounce(() => {
        if (!component.measurements) { return true }
        const pixelPosition = (
          component.scrollTop
          +
      ...
      disposables.add(editor.onDidDestroy(() => {
        element.removeEventListener('mousewheel', scrollPatch, { passive: true })
    ...
    this.disposables.add(atom.workspace.observePanes((pane) => {
      const disposables = new CompositeDisposable()
      disposables.add(pane.onDidChangeFlexScale((flexScale) => {
      ...
      disposables.add(pane.onDidDestroy(() => {
        disposables.dispose()
        this.disposables.remove(disposables)
      }))
