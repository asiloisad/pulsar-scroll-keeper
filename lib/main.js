...
module.exports = {
  ...
  activate() {
    ...
    this.bufferChangeSourceEditors = new WeakMap();
      ...
      atom.workspace.observeTextEditors((editor) => {
        ...
        disposables.add(
          editor.onWillInsertText(() => {
            this.bufferChangeSourceEditors.set(editor.getBuffer(), editor);
          }),
        );

        const didChangeDisplayLayerOriginal =
          component.didChangeDisplayLayer.bind(component);

        component.didChangeDisplayLayer = (changes) => {
          const buffer = editor.getBuffer();
          let sourceEditor = this.bufferChangeSourceEditors.get(buffer);

          if (!sourceEditor) {
            const activeEditor = atom.workspace.getActiveTextEditor();
            if (activeEditor && activeEditor.getBuffer() === buffer) {
              sourceEditor = activeEditor;
            }
          }

          let scrollDelta = 0;
          if (sourceEditor !== editor && component.measurements) {
            const firstVisibleRow =
              typeof component.getFirstVisibleRow === "function"
                ? component.getFirstVisibleRow()
                : component.getScrollTopRow();
          ...
          if (scrollDelta !== 0) {
            const lineHeight =
              typeof component.getLineHeight === "function"
                ? component.getLineHeight()
                : component.measurements?.lineHeight;

            if (lineHeight) {
              component.setScrollTop(
                component.getScrollTop() + scrollDelta * lineHeight,
              );
            }
          }

          if (this.bufferChangeSourceEditors.get(buffer) === sourceEditor) {
            Promise.resolve().then(() => {
              if (
                this.bufferChangeSourceEditors &&
                this.bufferChangeSourceEditors.get(buffer) === sourceEditor
              ) {
                this.bufferChangeSourceEditors.delete(buffer);
              }
            });
          }
        };
  ...
  deactivate() {
    ...
    this.bufferChangeSourceEditors = null;
  },
};
